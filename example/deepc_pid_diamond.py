...
for i in range(200):
    ...
    if i < 50:
        target = initial_position + np.array([-move_distance, -move_distance]) * (i / 50)
    elif i < 100:
        target = initial_position + np.array([-move_distance, -move_distance]) + np.array([move_distance, -move_distance]) * ((i - 50) / 50)
    elif i < 150:
        target = initial_position + np.array([0, -2 * move_distance]) + np.array([move_distance, move_distance]) * ((i - 100) / 50)
    else:
        target = initial_position + np.array([move_distance, -move_distance]) + np.array([-move_distance, move_distance]) * ((i - 150) / 50)
